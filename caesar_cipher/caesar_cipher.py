    ...
    def apply_shift(self, shift):
        ...
        for i in self.message_text:
            if i not in self.build_shift_dict(shift).keys():
                new_message_text.append(i)
                continue
            else:
                new_message_text.append(self.build_shift_dict(shift)[i])
...
class PlaintextMessage(Message):
    ...
    def change_shift(self, shift):
        ...
        (ie. self.encrypting_dict and message_text_encrypted).
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        on the message text. If s is the original shift value used to encrypt the message,
        then we would expect 26 - s to be the best shift value for decrypting it.

        Note: if multiple shifts are  equally good such that they all create
        the maximum number of you may choose any of those shifts (and their corresponding decrypted messages) to return

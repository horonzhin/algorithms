...
#
# Фабричный метод заключается в том, что у нас есть базовый класс (абстрактный класс,
# который характеризует общее поведение дочерних) и конкретные подклассы. И базовый и подклассы имеют одинаковые методы,
# но реализуют их по своему. За счет приведения типов мы можем прокинуть тип конкретного класса в базовый,
# т.е. в переменную с типом базового класса мы можем прокинуть переменную с типом конкретного класса
# и все будет работать. У нас есть общий метод, который реализуется конкретным дочерним классом этого семейства.
########################################################################################################################
